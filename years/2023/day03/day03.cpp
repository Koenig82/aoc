    ...
    if (rowIndex == 0) {
        for (size_t i = 0; i < input[rowIndex].size(); i++) {
            if (isdigit(input[rowIndex][i])) {
                ...
                while (i + 1 < input[rowIndex].length() &&
                       isdigit(input[rowIndex][i + 1])) {
                ...
                if (i < input[rowIndex].length() - 1 &&
                    (isSpecialChar(input[rowIndex + 1][i + 1]) ||
                     isSpecialChar(input[rowIndex][i + 1]))) {
                    addnumber = true;
                }
                if (addnumber) {
                    result.push_back(
                        stoi(input[rowIndex].substr(startpos, i + 1)));
                    addnumber = false;
                }
            }
        }
        ...
        for (size_t i = 0; i < input[rowIndex].size(); i++) {
            if (isdigit(input[rowIndex][i])) {
                ...
                if (isSpecialChar(input[rowIndex - 1][startpos]) ||
                    isSpecialChar(input[rowIndex + 1][startpos])) {
                    addnumber = true;
                }
                if (i < input[rowIndex].length() - 1 &&
                    (isSpecialChar(input[rowIndex - 1][i + 1]) ||
...
int Y2023d03::addNumbersAroundGear(const size_t gearPosRow,
    ...
    if (gearPosRow == 0) {
        ...
        if (gearPosCol > 0 && isdigit(input[gearPosRow - 1][gearPosCol - 1])) {
            if (gearPosCol > 1 &&
                isdigit(input[gearPosRow - 1][gearPosCol - 2])) {
                std::cout << "there is backwards search from top left"
                          << std::endl;
                while (gearPosCol - 1 - i > 0 &&
                       isdigit(input[gearPosRow - 1][gearPosCol - 1 - i])) {
                    i++;
                }
                startPos = gearPosCol - i;
                std::cout << input[gearPosRow - 1].substr(startPos, i)
                          << std::endl;
                i = 0;
            } else {
                std::cout << "there is forward search from top left"
                          << std::endl;
                while (isdigit(input[gearPosRow - 1][gearPosCol - 1 + i])) {
                    std::cout << "is digit:"
                              << input[gearPosRow - 1][gearPosCol - 1 + i]
                              << std::endl;
                    i++;
                }
        ...
        } else if (gearPosCol > 0 &&
                   isdigit(input[gearPosRow - 1][gearPosCol])) {
            std::cout << "there is forward search from top" << std::endl;
            while (isdigit(input[gearPosRow - 1][gearPosCol + i])) {
                std::cout << "is digit:"
                          << input[gearPosRow - 1][gearPosCol + i] << std::endl;
                i++;
            }
            startPos = gearPosCol;
            ...
            i = 0;
                   ...
                   isdigit(input[gearPosRow - 1][gearPosCol + 1])) {
